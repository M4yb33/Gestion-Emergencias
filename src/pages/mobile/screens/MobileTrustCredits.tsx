...
const MobileTrustCredits: React.FC<ScreenProps> = ({ navigate }) => {
  ...
  return (
    <div className="flex flex-col min-h-[700px]">
      <MobileHeader title="Confianza y CrÃ©ditos" hideBack />
        ...
        <div className="bg-gradient-to-br from-[#7b1c2e] to-[#1a2e5a] px-5 pt-6 pb-10">
          ...
          <div className="bg-white/10 rounded-xl p-3 mb-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-white/70 text-xs font-sans">PuntuaciÃ³n</p>
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {[{ label: "Reportes", value: "12" }, { label: "Confirmados", value: "8" }, { label: "PrecisiÃ³n", value: "67%" }].map((s) => (
        ...
        <div className="bg-neutral-50 -mt-6 rounded-t-3xl px-5 pt-5">
          <div className="bg-white border border-neutral-200 rounded-2xl p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-neutral-800 font-sans">CrÃ©ditos cÃ­vicos</h3>
              <button onClick={() => navigate("incentives")} className="text-xs text-primary font-medium cursor-pointer font-sans">
                Ver incentivos
              </button>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-bold text-xl text-neutral-900 font-sans">248 crÃ©ditos</p>
                <p className="text-xs text-neutral-500 font-sans">Acumulados</p>
              </div>
            </div>
          </div>

          <h3 className="text-sm font-semibold text-neutral-800 font-sans mb-2">Historial de puntos</h3>
          <div className="space-y-2">
            {history.map((item, i) => (
              <div key={i} className="flex items-center justify-between bg-white rounded-xl p-3 border border-neutral-200">
                <p className="text-xs text-neutral-600 font-sans">{item.action}</p>
                <p className={`font-bold text-sm font-sans ${item.color}`}>{item.credits}</p>
              </div>
            ))}
          </div>
        </div>
      </div>
      <MobileBottomNav active="trust" navigate={navigate} />
    </div>
  );
};
