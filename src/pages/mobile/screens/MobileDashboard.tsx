...
const recentReports = [
  { id: "R001", type: "Accidente de trÃ¡nsito", location: "Cra 50, MedellÃ­n", status: "Recibido" },
  { id: "R002", type: "Emergencia mÃ©dica", location: "Centro Comercial El Hueco", status: "Despachado" },
  { id: "R003", type: "Incendio", location: "Barrio Santa Cruz", status: "En revisiÃ³n" },
];

const MobileDashboard: React.FC<ScreenProps> = ({ navigate }) => {
  return (
    <div className="flex flex-col min-h-screen bg-neutral-50">
      <MobileHeader title="SafeAlert" hideBack />

      <div className="flex-1 pb-20 overflow-y-auto">
        <div className="px-4 pt-4 pb-3">
          <p className="text-xs text-neutral-500 font-sans">Buenos dÃ­as, Juan</p>
          <h1 className="text-2xl font-bold text-neutral-900 font-sans">Â¡SÃ© nuestros ojos!</h1>
        </div>

        <div className="grid grid-cols-4 gap-2 px-4 pb-4">
          {[
            { icon: "ðŸš¨", label: "Reportar", screen: "createReport" as const },
            { icon: "ðŸ“", label: "UbicaciÃ³n", screen: "location" as const },
            { icon: "ðŸŽ", label: "Incentivos", screen: "incentives" as const },
          ].map((item) => (
            <button
              key={item.label}
        ...
        <div className="mb-2 px-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-primary font-medium cursor-pointer font-sans" onClick={() => navigate("reportList")}>
              Ver todos
            </span>
          </div>
        ...
        <button
          onClick={() => navigate("whatsappChat")}
          className="w-full mt-4 bg-green-50 border border-green-200 rounded-xl p-3.5 flex items-center gap-3 cursor-pointer hover:bg-green-100 transition-colors mx-4"
        >
          <span className="text-2xl">ðŸ’¬</span>
          <div className="flex-1 text-left">
            <p className="text-xs font-semibold text-green-800 font-sans">Vincular WhatsApp</p>
            <p className="text-[10px] text-green-600 font-sans">Reporta tambiÃ©n por WhatsApp</p>
          </div>
          <span className="text-green-600 text-xs font-sans">â†’</span>
        </button>
      </div>
